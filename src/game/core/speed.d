...
import game.core.active : MapClickResult;
...
void maybeUpdatePhysics(Game game, in MapClickResult click) { with (game)
{
    ...
    if (pan.rewindOneTick) {
        ...
        if (click == MapClickResult.airClick
            || click == MapClickResult.goodAssignment
        ) {
            game.upd(1, DuringTurbo.no);
        }
    }
